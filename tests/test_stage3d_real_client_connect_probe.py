...
SCRIPT = ROOT / "scripts" / "smoke-real-client-connect-probe.sh"


def test_stage3d_real_client_connect_probe_documents_boundary():
    assert SCRIPT.exists(), "scripts/smoke-real-client-connect-probe.sh must exist"
    text = SCRIPT.read_text()

    assert "set -euo pipefail" in text
    assert "mktemp -d" in text
    assert "trap cleanup EXIT" in text
    assert 'kill -- "-${TARGET_PID}"' in text
    ...
    assert "TCP punch attempt" in text
    assert "Key mismatch" in text
    assert "invalid key" in text
    assert "SMOKE_REAL_CLIENT_CONNECT_PROBE_BOUNDARY" in text
    assert "two-client login/connect remains unverified" in text
