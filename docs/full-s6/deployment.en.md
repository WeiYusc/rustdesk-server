...
- Web Admin's **My → Address Book** online column is served by the API, not by hbbs 21115. It is derived dynamically from `peers.last_online_time` with the same short heartbeat window, so stale `address_books.online` database values are not authoritative for the Web panel. If the RustDesk client address book and Web Admin disagree, check both 21115 `OnlineRequest` and the Web Admin API response.
